...
const CircularProgress = ({ percent, radius, bgRingWidth, progressRingWidth, ringColor, ringBgColor,
  textFontSize, textFontWeight, clockwise, bgColor, startDegrees, currentCount, goal }) => {
  const commonStyles = {
    width: radius * 2,
    height: radius * 2,
    borderRadius: radius
  ...
  */
  ...
  return (
    <View style={[styles.container, { width: radius * 2, height: radius * 2 }]}>
      ...
      <Text style={[styles.display, { fontSize: textFontSize, fontWeight: textFontWeight }]}>{currentCount}</Text>
      <Text style={styles.subDisplay}>{goal}</Text>
    </View>
  );
}
